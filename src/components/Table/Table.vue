  ...
  methods: {
    ...
    setColGroupItemWidth() {
      ...
      this.$nextTick(() => {
        ...
        // 计算需要减去的宽度
        const subtractWidth = this.dragedRecord.filter((item) => item[0]).reduce((prev,now)=>{return now[1] + prev},0);
          ...
          if (i == this.activeDragIndex) {
            ...
            !isComputed &&
              this.$refs.colgroupItems[i].setAttribute(
                "width",
                (this.gnipTableScrollWidth - subtractWidth) /
